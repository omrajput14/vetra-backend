...
/**
 * Specialized AI agent responsible for veterinary treatment planning, medication recommendations,
 * and clinical follow-up guidance.
 */
@Component
public class TreatmentAgent implements AIAgent {

  private static final Logger log = LoggerFactory.getLogger(TreatmentAgent.class);
  public static final String AGENT_NAME = "TreatmentAgent";

  private final AIGateway aiGateway;
  private final AgentProperties agentProperties;

  /**
   * Constructs TreatmentAgent.
   *
   * @param aiGateway underlying AI gateway
   * @param agentProperties agent configuration properties
   */
  public TreatmentAgent(AIGateway aiGateway, AgentProperties agentProperties) {
  ...
  public AgentResponse execute(AgentRequest request) {
    String promptId =
        agentProperties != null
            ? agentProperties.getTreatmentPromptId()
            : "treatment.recommendation.v1";

    log.info("TreatmentAgent executing promptId={} for request={}", promptId, request.capability());
  ...
  public Set<AgentCapability> supportedCapabilities() {
    return Set.of(AgentCapability.TREATMENT);
  }
