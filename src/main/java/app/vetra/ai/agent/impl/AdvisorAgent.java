...
/**
 * Specialized AI agent responsible for conversational preliminary veterinary screening and clinical
 * guidance.
 */
@Component
public class AdvisorAgent implements AIAgent {

  private static final Logger log = LoggerFactory.getLogger(AdvisorAgent.class);
  public static final String AGENT_NAME = "AdvisorAgent";

  private final AIGateway aiGateway;
  private final AgentProperties agentProperties;

  /**
   * Constructs AdvisorAgent.
   *
   * @param aiGateway underlying AI gateway
   * @param agentProperties agent configuration properties
   */
  public AdvisorAgent(AIGateway aiGateway, AgentProperties agentProperties) {
  ...
  public AgentResponse execute(AgentRequest request) {
    String promptId =
        agentProperties != null && agentProperties.getAdvisorPromptId() != null
            ? agentProperties.getAdvisorPromptId()
            : "diagnosis.advisor.v1";

    log.info("AdvisorAgent executing promptId={} for capability={}", promptId, request.capability());
    ...
    AIRequest aiRequest =
        new AIRequest(
            ...
            request.imageUrl(),
            true, // Conversational advisor always bypasses standard static diagnostic cache
  ...
  public Set<AgentCapability> supportedCapabilities() {
    return Set.of(AgentCapability.ADVISOR);
  }
