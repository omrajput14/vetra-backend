...
/**
 * Specialized AI agent responsible for veterinary clinical triage and urgency classification.
 */
@Component
public class TriageAgent implements AIAgent {

  private static final Logger log = LoggerFactory.getLogger(TriageAgent.class);
  public static final String AGENT_NAME = "TriageAgent";

  private final AIGateway aiGateway;
  private final AgentProperties agentProperties;

  /**
   * Constructs TriageAgent.
   *
   * @param aiGateway underlying AI gateway
   * @param agentProperties agent configuration properties
   */
  public TriageAgent(AIGateway aiGateway, AgentProperties agentProperties) {
  ...
  public AgentResponse execute(AgentRequest request) {
    String promptId =
        agentProperties != null
            ? agentProperties.getTriagePromptId()
            : "triage.assessment.v1";

    log.info("TriageAgent executing promptId={} for request={}", promptId, request.capability());
  ...
  public Set<AgentCapability> supportedCapabilities() {
    return Set.of(AgentCapability.TRIAGE);
  }
